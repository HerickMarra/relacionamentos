...
trait LoveLanguageAI
{
    protected function generateIndividualAnalysis($user, $loveLanguage, $detailedAnswers = [])
    {
        ...
        $prompt = "Analise as escolhas reais deste usuário no teste: \n" .
                  "{$detailedText}\n\n" .
    ...
    protected function generateCompatibilityAnalysis($user1, $user2)
    {
        ...
        $prompt = "Compare estes dois perfis: \n" .
                  "{$user1->name}: " . json_encode($scores1) . "\n" .
                  "{$user2->name}: " . json_encode($scores2) . "\n\n" .
                  "Gere uma análise ULTRA-RESUMIDA (máximo 200 palavras no total).\n" .
                  "### 💫 Sincronia: 1 parágrafo curto.\n" .
                  "### 🎁 Para {$user1->name}: 1 dica curta.\n" .
                  "### 💝 Para {$user2->name}: 1 dica curta.\n" .
                  "### ⚡ Desafio: 1 frase direta.";
